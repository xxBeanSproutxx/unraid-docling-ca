...
- Keep `DOCLING_SERVE_ENABLE_REMOTE_SERVICES=true` during initial setup so model providers are reachable when artifacts are missing.
...
3. **Verify outbound network from the container**
   ...
   - RapidOCR artifacts are fetched from `www.modelscope.cn`; if that host is blocked, RapidOCR model files will stay missing.
...
5. **Check remote access policy**
   - `DOCLING_SERVE_ENABLE_REMOTE_SERVICES` must be `true` when bootstrap downloads are needed.
   - If you disable remote services later for security hardening, keep the populated model cache mounted so startup does not require downloads.
